 ...
 * abstraction:
 *
 *  - **JGallery** — a plain-file source: `StorageAccessDataSource.Factory` streams unencrypted local
 *    bytes through the §1.6 storage boundary (`core:playback`).
 *  - **CalcVault** — an encrypted source: `EncryptedVaultDataSource.Factory` decrypts vault blobs on
 *    demand, so no plaintext ever touches disk (stays CalcVault-side).
 *
 ...
 * Progressive covers the single-file local/vault case both apps use; a source that needs adaptive
