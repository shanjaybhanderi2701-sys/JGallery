...
android {
    ...
    // APP-637 keeps Firebase/Crashlytics DEBUG-ONLY, so the release-side CrashlyticsExtension
    // mapping-upload block (from APP-626's base) is intentionally NOT carried forward — release
    // ships zero Firebase. Only the sanitizer's JVM test hook below is retained.
    testOptions {
        unitTests.all {
